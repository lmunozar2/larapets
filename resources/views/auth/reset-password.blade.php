...
@section('content')
    <section class="bg-[#0006] text-white rounded-lg w-96 p-8 flex flex-col gap-4 items-center justify-center">
        <h1 class="flex gap-4 justify-center items-center text-4xl ">
            ...
            <form method="POST" action="{{ route('password.store') }}" class="card-body border-t-[1px]">
                <input type="hidden" name="token" value="{{ $request->route('token') }}">
                @csrf
                ...
                <button class="btn btn-outline hover:bg-[#fff6] hover:text-white mt-4">Reset</button>
